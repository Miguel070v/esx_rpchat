...
AddEventHandler('chatMessage', function(source, name, message)
	if string.sub(message, 1, string.len("/")) ~= "/" then
		...
		TriggerClientEvent('esx_rpchat:sendProximityMessage', -1, source, _U('oop_prefix', " | " .. name .. " [" .. source .. "]"), message, { 128, 128, 128 })
	end
	CancelEvent()
end)
...
RegisterCommand('twt', function(source, args, rawCommand)
	...
	TriggerClientEvent('chat:addMessage', -1, { args = { _U('twt_prefix', name .. " [" .. source .. "] "), args }, color = {91, 169, 255} })
	--print(('%s: %s'):format(name, args))
end, false)
...
RegisterCommand('badu', function(source, args, rawCommand)
	...
	if target.job ~= nil and target.job.name == "legal1" or target.job.name == "legal2" then
		args = table.concat(args, ' ')
		local name = GetPlayerName(source)
		if Config.EnableESXIdentity then name = GetCharacterName(source) end
...
RegisterCommand('oop', function(source, args, rawCommand)
	...
	local name = GetPlayerName(source)
	if Config.DisableESXIdentity then name = GetCharacterName(source) end
	TriggerClientEvent('esx_rpchat:sendProximityMessage', -1, source, _U('oop_prefix', " | " .. name .. " [" .. source .. "]"), args, { 128, 128, 128 })
	--print(('%s: %s'):format(name, args))
end, false)
...
RegisterCommand('me', function(source, args, rawCommand)
	...
	local name = GetPlayerName(source)
	if Config.EnableESXIdentity then name = GetCharacterName(source) end
	TriggerClientEvent('esx_rpchat:sendProximityMessage', -1, source, '^6[' .. source .. ']^6 ' .. args .. "")
end, false)
...
RegisterCommand('do', function(source, args, rawCommand)
	...
	local name = GetPlayerName(source)
	if Config.EnableESXIdentity then name = GetCharacterName(source) end
	TriggerClientEvent('esx_rpchat:sendProximityMessage', -1, source, '^3[' .. source .. '] ^3' .. args .. "")
	--print(('%s: %s'):format(name, args))
end, false)

--[[-- CORTAR PARA COMENTAR --]]--
...
RegisterCommand('dados', function(source, args, rawCommand)
	...
	TriggerClientEvent('esx_rpchat:sendProximityMessage', -1, source, '^2[' .. source .. ']^2 ' .. args .. "")
end, false)
